""" Main computational modules for PyMetrics.
    ...
    $Id: compute.py,v 1.3 2011/04/12 19:36:32 xavier Exp $
"""
...
class ComputeMetrics( object ):
    ...
    def __checkNumberOfExits( self ):
        ...
        if n > 0:
            ...
            #         exitStr,
            #         exits))
            msg = ("function %s has %d extra %s %s" % 
                    (self.__extractFQN( self.fqnFunction ),
                     n,
                     exitStr,
                     exits))
    ...
    def __call__( self, lex ):
        ...
        skipUntil = None
        tokCount = 0
        invalidToken = PyMetrics.mytoken.MyToken()
    ...
    def handleToken( self, tokCount, tok ):
        ...
        if tok.type == EMPTY or tok.type == NEWLINE:
            # >>> XAM
            self.processOneLineClassOrFunction(tok)
            # <<< XAM
            self.processStmt()
            return tokCount
        ...
        if self.checkForModuleDocString and numModDocStrings == 0:
            ...
            if msg and not self.pa.quietSw and not self.pa.quietModDocStringSw:
                print(msg)
    ...
    def processOneLineClassOrFunction(self, tok):
        """ Handle newline / functions defined one 1 line only
        """
        if tok.type == NEWLINE:
            if self.fcn : self.__doOneLineFunction()
            if self.cls : self.__doOneLineClass()

    def __doOneLineClass(self):
        """ Handle newlines and in case of classes
        defined one one single line, remove function/class info as needed."""
        # 1-  is the first token a 'class name'
        if self.cls and self.cls[0].type == NAME and self.cls[0].semtype == CLASSNAME:
            # 2- is the newline the one of the function definition
            if self.cls[0].row == self.cls[-1].row:
                # -> get the text
                texts = [ tok.text for tok in self.cls]
                line  = "".join(texts)
                #
                try:
                    elts = line.split(":")
                    ...
                    if not len(elts) > 1:
                        print("Class Definition erroneous: %s" % line)
                        return
                    ...
                    if len(elts) == 2:     
                        ...
                        # more than one ":" in the line!
                        regx_class = re.compile("^(.*)\((.*)\) *:(.*)$") # <class_def>, <class_bases>, <suite>
                        #
                        res = regx_class.search(line)
                        try:
                            class_def, class_bases, suite = res.groups()
                        except Exception as msg:
                            print("Class Definition erroneous: %s (%s)" % (line, str(msg)))
                            return
                    #
                    suite = suite.strip()
                    #  
                    if suite and (not suite.startswith('#')): # class defined with only 1 line!
                        self.blockDepth += 1 # in order to correct the wrong blockDepth -= 1
                        self.__doDedent()
                    else: # class defined on more than one line
                        return
                except Exception as msg:
                    print("Class Definition erroneous: %s (%s)" % (line, str(msg)))
                    return
    ...
    def __doOneLineFunction(self):
        """ Handle newlines and in case of functions
        defined one one single line, remove function/class info as needed."""
        # 1-  is the first token a 'function name'
        if self.fcn and self.fcn[0].type == NAME and self.fcn[0].semtype == FCNNAME:
            # 2- is the newline the one of the function definition
            if self.fcn[0].row == self.fcn[-1].row:
                # -> get the text
                texts = [ tok.text for tok in self.fcn]
                line  = "".join(texts)
                #
                try:
                    elts = line.split(":")
                    ...
                    if not len(elts) > 1:
                        print("Function Definition erroneous: %s" % line)
                        return
                    
                    if len(elts) == 2:
                        func_def = elts[0]
                        suite    = elts[1]
                    else: # more than one ":" in the line!
                        regx_func = re.compile("^(.*)\((.*)\) *:(.*)$") # <fun_def>, <fun_args>, <suite>
                        regx_func_annot = re.compile("^(.*)\((.*)\)( *->.*):(.*)$") # <fun_def>, <fun_args>, <annotation>, <suite>
                        #
                        res = regx_func.search(line)
                        res_annot = regx_func.search(line)
                        
                        suite = ''
                        
                        if res:
                            try:
                                func_def, fun_args, suite = res.groups()
                            except Exception as msg:
                                print("Function Definition erroneous: %s (%s)" % (line, str(msg)))
                                return
                        elif res_annot:
                            try:
                                func_def, fun_args, annotation, suite = res.groups()
                            except Exception as msg:
                                print("Function Definition erroneous: %s (%s)" % (line, str(msg)))
                                return     
                    #
                    suite = suite.strip()
                    #
                    if suite and (not suite.startswith('#')): # function defined with only 1 line!
                        self.blockDepth += 1 # in order to correct the wrong blockDepth -= 1
                        self.__doDedent()
                    else: # function defined on more than one line
                        return
                except Exception as msg:
                    print("Function Definition erroneous: %s (%s)" % (line, str(msg)))
                    return
    ...
    def __doDedentRemoveMsg(self):
        ...
        if msg and not self.pa.quietSw and self.pa.genBasicSw:
            print(msg)
        if self.pa.verbose > 0: 
