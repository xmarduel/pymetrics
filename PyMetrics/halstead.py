...
import math
import time
from PyMetrics.metricbase import MetricBase
...
class HalsteadMetric( MetricBase ):
    ...
    def display( self, cat=None ):
        ...
        module_keys = [x for x in keys if self.hsDict[x]['cat'] == '(M)']
        class_keys = [x for x in keys if self.hsDict[x]['cat'] == '(C)']
        function_keys = [x for x in keys if self.hsDict[x]['cat'] == '(F)']
