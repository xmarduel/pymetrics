""" Parsing classes.
    ...
    $Id: lexer.py,v 1.1.1.1 2009/10/10 08:22:31 xavier Exp $
"""
...
class Lexer:
    ...
    def parse( self, inFileName ):
        ...
        try:
            srcLines = fd.read()
            self.srcLines = srcLines.expandtabs()
        finally:
            fd.close()
    ...
    def __parseSource(self):
        ...
        self.pos = 0
        text = io.StringIO( self.srcLines )
        try:
            PyMetrics.tokenize.tokenize( text.readline, self )
    ...
    def __push(self, toktype, semtype, toktext, srow, scol, line):
        ...
        self.tokenlist.append(PyMetrics.mytoken.MyToken(type=toktype, semtype=semtype, text=toktext, row=srow, col=scol, line=line))
